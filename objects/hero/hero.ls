...
global.hero = do mixin object, ->
	...
	@weapons =
		{name: "normal"
		tag: "P"
		shot: ~> @shot shot, {dmg:2, side: @side, vel:v3(22*16*@dir,0,0)}},
		{name: "poke"
		tag: "PM"
		charge: 24
		cost: 1
	...
	climb: (dir) ~>
		...
		@ghost = @floats = true
		@vel.y = 16 * 4.5 * (if dir==\down then 1 else -1)
	...
	tick: after @tick, (dt)  ~> 
		...
		| @just_hurt! 		 => -@dir*2*16
		| @is_climbing! 	 => 0	
		| @is_sliding!		 => @dir*12*16
		| key.down key_right => 16*4.4
		| key.down key_left	 => -16*4.4
		| _					 => 0
		...
		if not @is_climbing! and not @is_sliding! then
			if key.down key_down and has_stair(@pos.x, @pos.y + 16) then
				@climb \down
			else if key.down key_up and has_stair(@pos.x, @pos.y - 16) then
				@climb \up
		else if @is_climbing! and not @just_climbed! and not has_stair(@pos.x,@pos.y + 16/4) then
	...
	is_almost_climbing: ~> @is_climbing! and !has_stair(@pos.x,@pos.y - 16/4) and has_stair(@pos.x,@pos.y + 16/4)
	...
	has_solid_ahead: ~>has_solid(@pos.x+@dir*16,@pos.y)
	has_solid_over: ~>not empty(filter (.solid), tree.get(@pos.x - @dir*16*1.1, @pos.y - 16*1.5, @pos.x + @dir*16*1.1, @pos.y - 16*0.5))
